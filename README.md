...
- 候选人问答 Agent 汇总简历、筛选、人工决策、面试、Offer、入职和企业知识库证据，支持可追溯 AI 问答与人工降级。
- AI 评测与错误案例库支持固定合成样本、离线评测运行、质量指标和错误案例处理。
...
索引任务支持幂等跳过、失败隔离、受控重试、强制重建和模型版本并存。删除简历批次或候选人档案时，关联向量分块会同步级联删除。当前候选人向量索引用于人才召回和后续 Agent 上下文检索，不替代硬条件判断和简历原文证据。企业招聘知识库 RAG 已作为独立 AI 专项落地，面向制度、岗位标准、面试评分和沟通话术等企业知识，支持文档上传、分块索引、权限过滤、引用返回，并接入简历评分、面试报告和候选人问答 Agent 场景。
...
F13 固定评测集包含 3 类职位和 30 份完全合成简历，均不对应真实个人。AI-05 已在此基础上提供管理员可用的 AI 评测与错误案例库页面，可初始化默认评测集、运行离线评测、查看样本结果并处理错误案例。评测集覆盖：
